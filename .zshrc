...
alias ghshell="ssh -i ~/.ssh/gh_ldap shell"
...
alias wol3="wakeonlan e4:43:4b:79:83:ec"
alias wol="wol1 && wol2"
...
function dip() {
        _print_container_info() {
            ...
            container_ipv4="$(docker inspect --format "{{range .NetworkSettings.Networks}}{{.IPAddress}} {{end}}" "$container_id")"
            container_ipv6="$(docker inspect --format "{{range .NetworkSettings.Networks}}{{.GlobalIPv6Address}} {{end}}" "$container_id")"
            container_status=$(docker inspect --format "{{.State.Status}}" "$container_id")
            printf "%-13s %-27s %-30s %-26s %-31s %-80s\n" "$container_id" "$container_name" "$container_status" "$container_ipv4" "${container_ipv6}" "${container_ports[*]}"
        }

        local container_id
        container_id="$1"
        printf "%-13s %-27s %-30s %-26s %-31s %-80s\n" 'Container Id' 'Container Name' 'Container Status' 'Container IPv4' 'Container IPv6' 'Container Ports'
